...
const tweetMode = require('./config/tweet-mode');
...
function requestHandler(resolve, reject, path, token, options) {
  ...
  if (this && this.options && this.options.tweet_mode) {
    options[tweetMode.key] = this.options.tweet_mode;
  }
  ...
  request.get(requestOptions, requestCallback.bind(this, resolve, reject));
}
...
function generateApiHandler(path, schema) {
  return generatedApiHandler.bind(this, path, schema);
}
